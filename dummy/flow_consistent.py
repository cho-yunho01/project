...
# 설정
# =========================
TOTAL_INVENTORY = 2000          # 최종 판매될 총 수량
BOX_CAPACITY = 20               # 박스당 20개 고정
...
for box_idx in range(1, TOTAL_BOXES + 1):
    ...
    order_code = make_order_code(order_created_at, box_idx)
    sales_code = make_sales_code(sold_at, box_idx)
    box_code = f"SE01-FA01-A1-{product_code}-{make_box_suffix(box_idx)}"
    ...
    for _ in range(BOX_CAPACITY):
        lot = f"{LOCATION_PREFIX}-{product_code}-{serial_seq:06d}"
