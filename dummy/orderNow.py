...
LOG_ID_START = 9000
SALES_ID_START = 9000
SALES_ITEM_ID_START = 9000

REGION = "SE01"
FACTORY = "FA01"
LINE = "A1"

OUTPUT_FILE = Path("full_flow_dummy_now.sql")

# 생성 날짜 범위
DATE_START = datetime(2026, 3, 1)
DATE_END = datetime(2026, 3, 19, 23, 59, 59)
SALE_END = datetime(2026, 3, 20, 23, 59, 59)
...
# 상품별 박스코드가 1000부터 시작되도록 999로 초기화
product_box_seq = {product_id: 999 for product_id in PRODUCTS.keys()}
...
def random_sale_time(inbound_time, delivery_date):
    sale_base_time = max(inbound_time, delivery_date)

    if sale_base_time < DATE_START:
        sale_base_time = DATE_START

    if sale_base_time > SALE_END:
        sale_base_time = SALE_END

    total_seconds = int((SALE_END - sale_base_time).total_seconds())

    if total_seconds <= 0:
        return sale_base_time

    random_seconds = random.randint(0, total_seconds)
    return sale_base_time + timedelta(seconds=random_seconds)
...
for key, units in grouped_units.items():
    ...
    while idx < total_units:
        ...
        sale_time = random_sale_time(inbound_time, delivery_date)
